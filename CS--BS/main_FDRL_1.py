...
from data_processing import send_weight_client
from data_processing import recive_weight_client
...
def main(args):
    ...
    for t in range(args.T):
        ...
        for i in range(len(samples)):
            request_i = samples.iloc[i, 1]  # movieID
        ...
        send_weight_client(w, len(request))
        global_weights=recive_weight_client()
        # tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        # dict_dump = pickle.dumps(local_weights)
...
if __name__ == '__main__':
    ...
    # # Sn=[20,30,40,50,60,70,80,90,100,110,120,130]
    # chr = []
    # for j in range(10):
    #     c_t = []
